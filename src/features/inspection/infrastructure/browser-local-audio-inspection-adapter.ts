...
export function createBrowserLocalAudioInspectionAdapter(
  ...
  return {
    inspect: async (assets) =>
      Promise.all(assets.map((asset) => inspectAsset(asset, fileRegistry))),
  };
}
...
async function inspectAsset(
  ...
  const inspection =
    file === undefined ? undefined : inspectHeader(await readHeader(file), asset.extension);
  const fallbackFormat = formatFromExtension(asset.extension);
...
function inspectWave(header: Uint8Array): HeaderInspection {
  ...
  while (offset + 8 <= header.byteLength) {
    ...
    if (id === 'fmt ' && chunkStart + Math.min(size, 16) <= header.byteLength) {
...
function inspectAiff(header: Uint8Array): HeaderInspection {
  ...
  while (offset + 8 <= header.byteLength) {
    ...
    if (id === 'COMM' && chunkStart + 18 <= header.byteLength) {
